...
function BadgeItem({ badge, showDot }: { badge: (typeof badges)[0]; showDot?: boolean }) {
  ...
  return (
    <>
      {showDot && <span className="shrink-0 w-1 h-1 rounded-full bg-muted-foreground/50 mx-3 lg:mx-6" aria-hidden />}
      <div className="flex shrink-0 items-center gap-2 min-w-[140px] sm:min-w-[160px] lg:min-w-[220px] px-2">
        <div className="w-6 h-6 lg:w-8 lg:h-8 rounded-sm bg-primary/10 flex items-center justify-center shrink-0">
          <Icon className="h-3 w-3 lg:h-4 lg:w-4 text-primary" />
        </div>
        <span className="text-xs sm:text-sm lg:text-base text-foreground whitespace-nowrap">{badge.title}</span>
      </div>
    </>
  )
}
...
export function TrustBadges() {
  ...
  return (
    <section className="py-2 sm:py-2.5 bg-muted border-y border-border overflow-hidden">
